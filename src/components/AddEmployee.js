...
function AddEmployee() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'photo') {
      ...
      setPhotoPreview(URL.createObjectURL(files[0])); // Set photo preview
    } else {
        ...
        <form onSubmit={handleSubmit} className="needs-validation" noValidate>
          ...
          <div className="form-group">
            ...
            <input
              type="file"
              name="photo"
              accept="image/*" // Accept images from all sources
              onChange={handleChange}
              required
              className="form-control"
              capture="environment" // Use "environment" for better mobile camera access
            />
            {photoPreview && (
